...
| **Component properties** `options`, `transport`, `debounceStore`, `dispatcher`, `urlResolver`, `logger`, `checks`, `environment` and the **accessors** `config()`, `kit()`, `rules()`, `keys()`, `transport()`, `submitter()`, `collector()`, `dispatcher()`, `debounceStore()`, `staging()`, `services()` | Names and types stay; new properties and accessors are only added. |
