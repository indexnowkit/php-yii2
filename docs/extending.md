...
`Hook\ObserverHelper`. `SubmitUrlsJob` is `Retry\WorkerOutcome` plus a delayed re-push of the rejected URLs (the delay
of `Retry\RetryPolicy` from `retry.*`, `Retry-After` first; yii2-queue's `canRetry()` only concerns exceptions,
see [queue.md](queue.md)). The options of every `indexnow/<action>` come from `Console\Definitions`
